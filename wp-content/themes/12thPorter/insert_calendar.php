<?php
	...
	$XML_CALENDAR_FEED = 'http://www.ticketweb.com/snl/EventAPI.action?version=1&method=xml&key=JnuDjlMLm1u8gpmhnZ5E&venueId=25638&resultsPerPage=999';
...
<!-- 							<img src="<?php echo $event->thumbURL ?>" alt="<?php echo substr($event->name, 0, 40) ?>"> -->
							<br/><?php echo substr($event->name, 0, 30) ?>
						</a>
					</li>
					<?php endforeach ?>
				</ul>
	...
	<?php
		$DEA_year_month = $DateEventArray[(int)$targetYear][(int)$targetMonth];
		$DEA_year_month = (is_array($DEA_year_month)) ? $DEA_year_month : Array();
		foreach ($DEA_year_month as $day) :
